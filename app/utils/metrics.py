...
def calculate_sleep_metrics(data: SleepData, stages: Optional[List[SleepStage]] = None) -> SleepMetrics:
    ...
    if stages:
        ...
        sleep_efficiency = (total_sleep_minutes / total_duration_minutes) * 100 if total_duration_minutes > 0 else 0.0
        ...
        time_to_fall_asleep_minutes = _estimate_sleep_latency(data)
    ...
    sleep_quality = _determine_sleep_quality(
        ...
        deep_sleep_percentage=(deep_sleep_minutes / total_duration_minutes) * 100 if total_duration_minutes > 0 else 0,
        rem_sleep_percentage=(rem_sleep_minutes / total_duration_minutes) * 100 if total_duration_minutes > 0 else 0,
...
def _count_awakenings(stages: List[SleepStage]) -> tuple:
    ...
    for stage in sorted_stages:
        if not sleep_started:
            ...
            if stage.stage_type == SleepStageType.AWAKE:
                awakenings += 1
                awake_time_minutes += (stage.end_time - stage.start_time).total_seconds() / 60.0
...
def _estimate_sleep_latency(data: SleepData) -> float:
    """Estimate time to fall asleep from raw data."""
    # Extract movement data for the first 30 minutes
    movement_df = _extract_movement_data(data)
    
    if movement_df.empty:
        return 15.0  # Default estimate
        
    # Use only data from the first 30 minutes
    start_time = data.start_time
    movement_df = movement_df[movement_df["timestamp"] <= start_time + timedelta(minutes=30)]
    
    if "magnitude" not in movement_df.columns and all(col in movement_df.columns for col in ["x", "y", "z"]):
        movement_df["magnitude"] = np.sqrt(
            movement_df["x"]**2 + movement_df["y"]**2 + movement_df["z"]**2
        )
    
    if "magnitude" in movement_df.columns:
        # Use a rolling window to find when movement drops significantly
        window_size = min(100, len(movement_df) // 4)  # Reasonable window size
        if window_size > 0:
            movement_df["rolling_mean"] = movement_df["magnitude"].rolling(window=window_size, min_periods=1).mean()
            
            # Find when rolling mean drops below threshold
            sleep_onset_threshold = settings.SLEEP_DETECTION_THRESHOLD
            sleep_onset_samples = movement_df[movement_df["rolling_mean"] < sleep_onset_threshold]
            
            if not sleep_onset_samples.empty:
                first_sleep_time = sleep_onset_samples.iloc[0]["timestamp"]
                sleep_latency = (first_sleep_time - start_time).total_seconds() / 60.0
                return min(30.0, max(0.0, sleep_latency))  # Keep within reasonable bounds
